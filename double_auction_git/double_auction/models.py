...
class Constants(BaseConstants):
    ...
    price_max_numbers = 25
    price_digits = 0
    initial_quantity = 1
    seller_cost_range = (0, 0)
    buyer_value_range = (20, 20)
    endowment = 200
    ...
    def presence_check(self):
        ...
        if self.no_buyers_left():
            ...
            msg = {'market_over': True,
                   'over_message': 'Keine Käufer mehr verfügbar'}

        if self.no_sellers_left():
            self.active = False
            self.save()
            msg = {'market_over': True,
                   'over_message': 'Keine Verkäufer meht verfügbar'}
        return msg
    ...
    def presence_check(self):
        ...
        if not self.is_active():
            if self.role() == 'buyer':
                if self.endowment <= 0:
                    msg = {'market_over': True,
                           'over_message': 'Kein Geld für Handel mehr übrig'}
                if not self.has_free_slots():
                    msg = {'market_over': True,
                           'over_message': 'Ihr Portfolio ist voll'}
            else:
                msg = {'market_over': True,
                       'over_message': 'Keine Aktien mehr verfügbar'}
        return msg
